...
>> SimpleController은 HttpServlet 파일을 상속하고 있다.
>> SimpleController.java >> 웹에 사용되는 객체를 사용 가능 >> 일반 자바 파일이 아니라 웹용 자바파일(서블릿)
...
에서
	<servlet-name>simplecontroller</servlet-name>
  	...
  	2. 	<servlet-name>simplecontroller</servlet-name>
  		<servlet-class>com.SimpleController</servlet-class> 설정 근거로
  		com.SimpleController.java 컴파일하고 실행 합니다...
  	3. 다음 똑같은 요청 들어오면 실행파일 있으면 실행 합니다...
  	
  	서블릿 조건: 일반 java 파일이 extends HttpServlet 상속 >> 웹 요청(request), 웹 응답(response)
  	-servelt url 요청 바로 할 수 없다 >> 요청 >> mapping  >> xml or annotation
  	
  	1. web.xml (구방식)
  	2. @@WebServlet (현재 ..)  ex) WebServlet("/simple") 매핑
  	
  	protected void doGet() 함수
  	ex) http://localhost:8090/WebServlet_1/simple GET 방식
  	
  	protected void doPost() 함수
  	ex) http://localhost:8090/WebServlet_1/simple (form 만들고 method=post 지정)
  	명시적인 호출에 의해서 동작 ... 일반적인 ...
  	
  	2개의 함수는 어떤 사건이 발생하는 자동 호출 되도록 만들어짐
  	
  	EX) Javascript 이벤트 ... 자동 함수 호풀 (onload... 사건이 발생하면 핸들러 함수 자동 호출)
...
public class SimpleController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//MVP 패턴 흉내 ...JSP 있는 시대입니다 *******
		//4. JSP (view) 데이터 출력 하려면 >> servlet 가지고 있는 자원을 >> View에게 전달 (JSP)
		...
		dis.forward(request, response);//현재 내가 가지고 있는 request 객체의 주소값을 forward parameter로 시킨다.
